...
type Message = {
  ...
  createdAt: string
  lu: boolean
  envoyeur: { nom: string; prenom: string }
}
...
function MessagesContent() {
  ...
  useEffect(() => {
    fetch('/api/me').then(r => r.json()).then(d => setMoi(d.user))
    fetchConversations()
  }, [fetchConversations])
  ...
  useEffect(() => {
    ...
    if (conv) {
      ...
      fetch('/api/user/' + avecUserId).then(r => r.json()).then(data => {
        if (data.user) {
          setSelectedUser({
            userId: data.user.id,
            nom: data.user.nom,
  ...
  useEffect(() => {
    if (visite === 'true' && voiture && selectedUser && moi && !visiteSent) {
      setVisiteSent(true)
      fetch('/api/messages', {
        method: 'POST',
        ...
        body: JSON.stringify({
          ...
          contenu: 'Bonjour, je suis intéressé(e) par votre ' + voiture + '. Pouvez-vous me proposer un moment pour une visite ? Merci.',
        }),
      }).then(() => {
  ...
  const handleSend = async (e: React.FormEvent) => {
    ...
    try {
      await fetch('/api/messages', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ destinataireId: selectedUser.userId, contenu }),
      })
      setContenu('')
      fetchMessages(selectedUser.userId)
      fetchConversations()
  ...
  const handlePartagerPosition = () => {
    ...
    navigator.geolocation.getCurrentPosition(
      async (position) => {
        const lien = 'https://maps.google.com/?q=' + position.coords.latitude + ',' + position.coords.longitude
        await fetch('/api/messages', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({ destinataireId: selectedUser.userId, contenu: 'MAPS:' + lien }),
        })
        fetchMessages(selectedUser.userId)
        fetchConversations()
      },
  ...
  const getLienMaps = (texte: string): string => {
    const index = texte.indexOf('MAPS:')
    return index !== -1 ? texte.substring(index + 5) : '#'
  }
            ...
            <div className="flex-1 flex flex-col">
              <div className="bg-white border-b border-black/10 px-5 py-4 flex items-center gap-3">
                ...
                <button
                  ...
                  className="px-3 py-1.5 rounded-lg bg-[#2A7A4B]/10 text-[#2A7A4B] text-xs font-semibold border border-[#2A7A4B]/25 hover:bg-[#2A7A4B]/20 transition-all"
                >
                  📍 Ma position
                </button>
              </div>
              ...
              <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
                ...
                {messages.map((msg) => {
                  ...
                  const estMaps = msg.contenu.startsWith('MAPS:')
                  const lienMaps = estMaps ? getLienMaps(msg.contenu) : '#'
                  const couleur = estMoi ? 'bg-[#C17B2E] text-white rounded-br-sm' : 'bg-white text-[#1A1208] border border-black/10 rounded-bl-sm'
                  return (
                    <div key={msg.id} className={'flex ' + (estMoi ? 'justify-end' : 'justify-start')}>
                      <div className={'max-w-xs px-4 py-2.5 rounded-2xl text-sm ' + couleur}>
                        {estMaps ? (
                          <div>
                            <p className="mb-1">📍 Localisation partagée</p>
                            <a href={lienMaps} target="_blank" rel="noopener noreferrer" className="underline text-xs">
